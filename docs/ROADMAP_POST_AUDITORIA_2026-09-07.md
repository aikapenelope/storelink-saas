...
- **Cambios (APROBADO por el dueño; DDL aplicado vía Supabase MCP/execute_sql, nunca pooler):**
  ...
  ```
  Policies `payload_server_full_access` (`TO postgres`, patrón idéntico a las otras 26 tablas del repo) por consistencia: `postgres` tiene `rolbypassrls=true` (verificado), así que la app queda intacta; para `anon`/`authenticated` = deny-all (ninguna policy les aplica). No se registra en `payload_migrations` (Payload no modela RLS y `prodMigrations` no lo ejecutará); se documenta aquí.
- **AC:** [x] advisors de seguridad → **0 errores** (verificado: lint 0013 eliminado; solo quedan 2 INFO 0008 de `payload_jobs`/`payload_jobs_log`, intencionales y preexistentes); [x] smoke CRM: CRUD completo sobre `customers_purchase_history` como `postgres` (rol del pooler de Payload) OK en tx con ROLLBACK, y lectura de `customers` de producción intacta (totales CRM correctos).
