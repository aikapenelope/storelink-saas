    ...
    return NextResponse.json(
      {
        ...
        queued: true,
        jobId: job.id,
        message: `Sincronización en cola para ${tenantResult.docs[0].name} (${describeCatalogLimit(tenantResult.docs[0].plan)}; los re-sync de SKUs existentes no consumen cupo). Se reflejará en el catálogo en unos segundos.`,
      },
      { status: 202 }
    );
  } catch (err) {
